...
import { ArrowLeft, Plus, Search, Pencil, Package, CalendarDays, DollarSign, Target } from 'lucide-react'
...
export default function CollectionDetailPage() {
  ...
  if (!collection) return <div className="text-dark/40">Collection introuvable.</div>

  const filtered = (products ?? []).filter((p) => {
    if (!search) return true
    const q = search.toLowerCase()
    return p.name?.toLowerCase().includes(q) || p.reference?.toLowerCase().includes(q) || p.family?.toLowerCase().includes(q)
  })
  ...
  return (
    <div className="space-y-6 max-w-5xl">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className="font-mono text-xs text-dark/30 uppercase tracking-wider">{collection.code}</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-dark/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-dark/40">{collection.target_refs ? `/ ${collection.target_refs} cible` : 'références'}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-base font-semibold text-dark">{formatCurrency(collection.budget)}</p>
      ...
      <div>
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)}
                placeholder="Rechercher…" className="input-field pl-8 py-1.5 w-48" />
            </div>
            {CAN_EDIT.includes(user?.role) && (
              <button onClick={() => setProductOpen(true)} className="btn-primary">
        ...
        {loadingProducts ? (
          ...
          <EmptyState
            ...
            action={CAN_EDIT.includes(user?.role) && !search ? (
              <button onClick={() => setProductOpen(true)} className="btn-primary">
                <Plus size={14} /> Nouveau produit
              </button>
            ) : undefined}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-dark/5">
                  {['Référence', 'Produit', 'Type', 'Famille', 'Statut', 'Prix cible'].map((h) => (
              ...
              <tbody className="divide-y divide-dark/5">
                {filtered.map((p) => (
                  <tr key={p.id} className="hover:bg-cream transition-colors">
                    ...
                    <td className="px-4 py-3 text-dark/50">{p.type?.replace('_', '-') ?? '—'}</td>
                    <td className="px-4 py-3 text-dark/50">{p.family ?? '—'}</td>
                    <td className="px-4 py-3"><Badge status={p.status} /></td>
                    <td className="px-4 py-3 text-dark/70">
                      {p.target_retail_price ? formatCurrency(p.target_retail_price) : '—'}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <CollectionFormModal open={editOpen} onClose={() => setEditOpen(false)} collection={collection} />
      <ProductFormModal open={productOpen} onClose={() => setProductOpen(false)} collectionId={id} />
    </div>
  )
}
